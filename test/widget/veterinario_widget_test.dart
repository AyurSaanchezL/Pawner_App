...
void main() {
  group('CitaDetailSheet', () {
    ...
    testWidgets('muestra motivo y fecha correctamente', (tester) async {
      ...
      await tester.pumpWidget(_wrap(
        Builder(builder: (ctx) => CitaDetailSheet(cita: cita, mascota: _mascota())),
      ));
          ...
          builder: (ctx) => Scaffold(
            body: ElevatedButton(
              onPressed: () => showModalBottomSheet(
                ...
                builder: (_) => CitaDetailSheet(cita: _cita(), mascota: _mascota()),
              ),
  ...
  group('AddCitaSheet', () {
    ...
    testWidgets('muestra los hint texts de los 3 campos de texto', (tester) async {
      await tester.pumpWidget(buildSheet());
      await tester.pump();
    ...
    testWidgets('submit sin motivo muestra "Campo obligatorio"', (tester) async {
      await tester.pumpWidget(buildSheet());
      await tester.pump();
    ...
    testWidgets('"Agendar Cita" está habilitado cuando el formulario no está cargando', (tester) async {
      await tester.pumpWidget(buildSheet());
      await tester.pump();
    ...
    testWidgets('muestra 5 ChoiceChips de timing cuando la notificación está activa', (tester) async {
      await tester.pumpWidget(buildSheet());
      await tester.pump();
      for (final label in ['1h antes', '5h antes', '1 día antes', '1 semana antes', 'Personalizado']) {
        expect(find.text(label), findsOneWidget, reason: 'Chip "$label" debe estar visible');
      }
    });

    testWidgets('ocultar chips de timing al apagar el switch de notificación', (tester) async {
      await tester.pumpWidget(buildSheet());
      await tester.pump();
    ...
    testWidgets('seleccionar "Personalizado" muestra el selector de fecha personalizada', (tester) async {
      await tester.pumpWidget(buildSheet());
      await tester.pump();
    ...
    testWidgets('ActionChips aparecen si vetConfig tiene veterinarios', (tester) async {
      await fakeFirestore
          .collection('Familias')
          .doc('fam-1')
          .collection('Mascotas')
          ...
          .set({
        'veterinarios': [
          {'nombre': 'Dr. García', 'telefono': '600111222', 'numColegiado': ''}
        ],
        'alergias': [],
      });
    ...
    testWidgets('tap en ActionChip rellena el campo veterinario', (tester) async {
      await fakeFirestore
          .collection('Familias')
          .doc('fam-1')
          .collection('Mascotas')
